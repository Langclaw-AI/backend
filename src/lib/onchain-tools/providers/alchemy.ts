...
export async function getTokenBalances(
  ...
  const sourceUrl = buildRpcUrl(options.chain);
  const data = await rpc(sourceUrl, "alchemy_getTokenBalances", [
    walletAddress,
    "erc20",
    { maxCount: 20 },
  ], options.signal);
...
function buildRpcUrl(chain: string) {
  ...
  return `https://${network}.g.alchemy.com/v2/${requireEnv("ALCHEMY_API_KEY")}`;
}
