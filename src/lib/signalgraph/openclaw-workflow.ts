    ...
    buildPrompt: (skillText) =>
      buildPrompt({
        ...
        payload: {
          ...
          sources: compactSources(sources),
          proofConstraint:
            "Return prepared proof inputs only. Do not claim real 0G Storage upload or chain anchoring.",
        },
        requiredShape: {
...
function buildFallbackPlannerOutput(topic: string): PlannerOutput {
  return {
    ...
    providerPlan: [
      {
        provider: "X",
        query: `${topic} agent AI Web3 0G product launch builder`,
        purpose: "Find public launch, builder, and ecosystem discussion signals.",
      },
      {
        provider: "GitHub",
      ...
      {
        provider: "Tavily",
        query: `${topic} 0G OpenClaw AI agents infrastructure documentation`,
