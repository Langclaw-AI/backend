...
With the default setting, Langclaw probes the OpenClaw CLI, runs the reasoning steps through OpenClaw, returns execution metadata for each step, and asks the OpenClaw-connected model to synthesize the final chat answer when 0G Compute Router is not enabled.
...
OPENCLAW_STEP_TIMEOUT_SECONDS=60
OPENCLAW_AI_SYNTHESIS=true
OPENCLAW_AGENT_SESSION_ID=langclaw-final-answer
...
- `src/lib/langclaw/zero-g-compute.ts` calls 0G Compute Router for final-answer inference when `OG_COMPUTE_ENABLED=true`.
- `src/lib/langclaw/zero-g-proof.ts` uploads the canonical evidence bundle to 0G Storage when `OG_STORAGE_ENABLED=true`.
- `src/lib/langclaw/zero-g-proof.ts` anchors the final brief hash through `LangclawRegistry` when `OG_CHAIN_ENABLED=true`.
